<template>
  <div
    ...
    :class="{
      'impossible-to-itemMove':
        sectionStore.itemMove.items[props.itemIndex].itemMoveAmount >
          props.itemDetail.amount ||
        (sectionStore.itemMove.items[props.itemIndex].reasonName === 'other' &&
          sectionStore.itemMove.items[props.itemIndex].additionalReasonName ===
            ''),
    }"
  >
    ...
    <div class="list-item-body-detail q-mt-sm">
      ...
      >
        <q-tooltip
          ...
          :offset="[5, 3]"
        >
          Переміщення перебільшує наявність
        </q-tooltip>
        Кількість для списання:
        {{ itemMoveAmountOfItem }}
      </div>
      <div
        class="q-mt-sm"
        :class="{
          'warning-text':
            sectionStore.itemMove.items[props.itemIndex].reasonName ===
              'other' &&
            sectionStore.itemMove.items[props.itemIndex]
      ...
      >
        <q-tooltip
          ...
          :offset="[5, 3]"
        >
          Вкажіть причину переміщення
        </q-tooltip>
        Причина: {{ itemMoveReasonLabel }} {{ reasonDetail }}
      </div>
    </div>
  </div>
  <div class="validator-container col-12 q-mt-none q-ml-lg q-pt-none">
    <q-input
      ...
      :rules="[
        () =>
          sectionStore.itemMove.items[props.itemIndex].itemMoveAmount <=
            props.itemDetail.amount || 'Переміщення перебільшує наявність',
        () =>
          !(
            sectionStore.itemMove.items[props.itemIndex].reasonName ===
              'other' &&
    ...
    <q-card style="width: 100vw; max-width: 400px">
      ...
      <q-form @submit.prevent="submitAmount">
        <q-card-section class="col-12">
          <q-input
            outlined
            v-model="tempVariables.itemMoveAmount"
            label="Вкажіть кількість переміщення"
            mask="######"
            :rules="[
              (val) =>
...
function removeItem(itemIndex) {
  sectionStore.itemMove.items.splice(itemIndex, 1);
}

function showAmountDialog() {
  showEditAmountDialog.value = true;
  tempVariables.itemMoveAmount =
    sectionStore.itemMove.items[props.itemIndex].itemMoveAmount;
}
function submitAmount() {
  ...
  sectionStore.itemMove.items[props.itemIndex].itemMoveAmountCustomMode = true;
}
...
function showReasonDialog() {
  ...
  tempVariables.reasonDetail =
    sectionStore.itemMove.items[props.itemIndex].reasonDetail;
}
function submitReasonDialog() {
  ...
  sectionStore.itemMove.items[props.itemIndex].reasonCustomMode = true;
}

const itemMoveAmountOfItem = computed(() => {
  let itemMoveAmount =
    sectionStore.itemMove.items[props.itemIndex].itemMoveAmount;
  return itemMoveAmount === "" ? "-" : itemMoveAmount;
});

const itemMoveReasonLabel = computed(() => {
  let label = "";
  let reason = sectionStore.itemMove.items[props.itemIndex].reasonName;
...
const reasonDetail = computed(() => {
  let reasonDetail = sectionStore.itemMove.items[props.itemIndex].reasonDetail;
  return reasonDetail === "" ? "" : `(${reasonDetail})`;
});

watch(
  () => sectionStore.itemMove.itemMoveAmount,
  (newValue) => {
    if (
      !sectionStore.itemMove.items[props.itemIndex].itemMoveAmountCustomMode
    ) {
      sectionStore.itemMove.items[props.itemIndex].itemMoveAmount = newValue;
    }
  }
);

watch(
  () => sectionStore.itemMove.reasonName,
  (newValue) => {
    if (!sectionStore.itemMove.items[props.itemIndex].reasonCustomMode) {
      sectionStore.itemMove.items[props.itemIndex].reasonName = newValue;
    }
  }
);

watch(
  () => sectionStore.itemMove.additionalReasonName,
  (newValue) => {
    if (!sectionStore.itemMove.items[props.itemIndex].reasonCustomMode) {
      sectionStore.itemMove.items[props.itemIndex].additionalReasonName =
        newValue;
...
watch(
  () => sectionStore.itemMove.reasonDetail,
  (newValue) => {
    if (!sectionStore.itemMove.items[props.itemIndex].reasonCustomMode) {
      sectionStore.itemMove.items[props.itemIndex].reasonDetail = newValue;
    }
  }
);
</script>
